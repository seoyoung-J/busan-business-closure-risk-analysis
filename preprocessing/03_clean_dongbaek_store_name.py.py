...
def process_business_names(df):
    df_bizname_pd = df.select("uid", "가맹점명").toPandas()

    df_bizname_pd["정제된가맹점명"] = df_bizname_pd["가맹점명"].apply(clean_bizname_final)
    df_bizname_pd["정제결과"] = df_bizname_pd["정제된가맹점명"]

    valid_all_df = df_bizname_pd[df_bizname_pd["정제결과"].isnull()][["uid", "가맹점명", "정제된가맹점명", "정제결과"]].copy()

    valid_all_df["정제된가맹점명"] = valid_all_df["가맹점명"].apply(recover_english_in_parentheses)
    valid_all_df["정제결과"] = valid_all_df["정제된가맹점명"]

    valid_all_df = valid_all_df[
        (valid_all_df["정제된가맹점명"].notnull()) & 
        (valid_all_df["정제된가맹점명"].str.lower() != "null")
    ].copy()

    cleaned_business_name_df = df_bizname_pd.copy()
    cleaned_business_name_df.set_index("uid", inplace=True)
    valid_all_df.set_index("uid", inplace=True)
    cleaned_business_name_df.update(valid_all_df[["정제된가맹점명", "정제결과"]])
...
def process_final_business_names(df, brand_top_n=500, min_brand_freq=500, words_to_remove=None):
    ...
    # 보류 데이터 전처리
    df_hold["가맹점명_cleaned"] = df_hold["가맹점명"].apply(clean_parentheses_ju)

    # 대표상호 추출
    df_hold["대표상호"] = df_hold["가맹점명_cleaned"].apply(lambda x: extract_from_dict_better(x, brand_dict_sorted))
    ...
    # 정제된가맹점명, 정제결과 갱신
    df_hold_valid["정제된가맹점명"] = df_hold_valid["대표상호"]
    df_hold_valid["정제결과"] = df_hold_valid["대표상호"]

    # 필요한 컬럼만 유지
    df_hold_valid = df_hold_valid[["uid", "가맹점명", "정제된가맹점명", "정제결과"]].copy()
...
def preprocess_for_embedding(df):
    ...
    df["정제된가맹점명"] = df["정제된가맹점명"].replace(brand_standard_mapping)
    df["정제결과"] = df["정제결과"].replace(brand_standard_mapping)

    # 소문자 변환
    df["정제된가맹점명"] = df["정제된가맹점명"].str.lower()
    df["정제결과"] = df["정제결과"].str.lower()

    # Null 제거
    df = df[df["정제된가맹점명"].notnull()].copy()

    # 숫자만 존재하는 경우 처리
    df = df[df["정제된가맹점명"].apply(is_only_number_except_year)].copy()

    # 특수문자 정리
    df["정제된가맹점명"] = df["정제된가맹점명"].apply(clean_special_tokens)
    df["정제결과"] = df["정제결과"].apply(clean_special_tokens)

    # 1글자 이하 제거
    df = df[df["정제된가맹점명"].apply(lambda x: len(x.strip()) > 1)].copy()

    return df

# MAGIC %md
# MAGIC ### 최종 전처리 완료된 df 생성  
df = spark.table("gold.busan.dongbaek_addr_final") 
...
schema = StructType([
    ...
    StructField("가맹점명", StringType(), True),        
    StructField("정제된가맹점명", StringType(), True),
    StructField("정제결과", StringType(), True),
])

final_business_name = spark.createDataFrame(final_business_name_df, schema=schema)

final_business_name.write.format("delta").mode("overwrite").saveAsTable("gold.busan.mois_dongbaek_clean_addr_v2")